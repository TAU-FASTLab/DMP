...
function generatePDF() {
  ...
  function addRiskTableRows(tableId) {
      ...
      rows.forEach((row, index) => {
          if (index > 0) addText('');
          const risk = row.querySelector(`#risk${tableId.slice(-2)}_${index + 1}`);
          const likelihood = row.querySelector(`#likelihood${tableId.slice(-2)}_${index + 1}`);
          const severity = row.querySelector(`#severity${tableId.slice(-2)}_${index + 1}`);
          const mitigation = row.querySelector(`#mitigation${tableId.slice(-2)}_${index + 1}`);
          const remainingRisk = row.querySelector(`#remainingRisk${tableId.slice(-2)}_${index + 1}`);
  ...
  addSectionTitle('1.1.1. Name of the research study or other identifier');
  ...
  addFieldText('Number of research subjects', 'dataMinimisation');
  addFieldText('Planned duration of the research study', 'transparency');
  ...
  addText('The term personal data refers to all information relating to an identified or identifiable natural person.'+
  'Natural persons are considered identifiable if they can be identified (directly or indirectly) in particular by '+
  'reference to an identifier such as a name, an identification number, location data, an online identifier or one or'+
  'more factors specific to their physical, physiological, genetic, mental, economic, cultural or social identity.');
  addText('What types of personal data will be processed during the research study? Describe the personal data that will'+
  'be processed (types of personal data), whose personal data will be processed (groups of data subjects) and how or from'+
  'whom personal data will be collected (sources of personal data). You must also indicate the purposes for processing '+
  'personal data (purpose of processing) and the lawful basis for each processing activity (lawful basis of processing)');
  ...
  addText('The term special category personal data refers to data that reveals an individual racial or ethnic origin,'+
  'political opinions, religious or philosophical beliefs or trade union membership, genetic or biometric data '+
  '(when used for identification purposes), health data, or data concerning his or her sexual behaviour or orientation.');
  ...
  addFieldText('Description of processing activities', 'integrityMeasures');
  ...
  /*addFieldText('Details about the categories and sources of personal data', 'codeOfConduct');*/
  addFieldText('Details about processing special category personal data', 'consent');
  addFieldText('Special categories of personal data and data concerning criminal convictions', 'dataSubject');
  addFieldText('Description of processing activities', 'integrityMeasures');
  addFieldText('Codes of conduct', 'jointControllersDetails');
  ...
  addSectionTitle('2.7. Integrity, confidentiality and availabilitys');
  ...
  addText('Describe the measures for ensuring integrity and confidentiality throughout the data lifecycle');
  ...
  addText('Assess the risks that your processing activities may pose to data subjects, especially the risks arising from unauthorized access, loss, or alteration of personal data:unauthorised disclosure of or access to personal data,'
  +'unintentional or unauthorised destruction or modification of personal data, the loss or alteration of personal data.');
  addText('How would you rate the severity of the possible harm or damage if the risks are realised? How would you rate the likelihood of the risks? Potential risks include:'+
  'unauthorised re-identification of pseudonymised data or loss of control over the use of personal data,'+
  'identity theft or fraud, financial loss,'+
  'other social disadvantage, such as reputational damage, or'+
  'physical harm (especially when processing special category data).');
  ...
  addSectionTitle('4.1. Comments by the data protection officer');
  addFieldText(' Name', 'dpoName');
  addFieldText('Date', 'dpoDate');
  addFieldText('Comments', 'dpoComments');
  addFieldText('4.2. Comments by other stakeholders - Stakeholder group');
  addSectionTitle('Stakeholder group');
  addFieldText('Stakeholder group', 'stakeholderGroup');
